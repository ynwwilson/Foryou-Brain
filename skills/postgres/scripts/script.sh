#!/usr/bin/env bash
# postgres — Postgres reference tool. Use when working with postgres in database contexts.
...
show_help() {
    cat << 'HELPEOF'
postgres v$VERSION — Postgres Reference Tool

Usage: postgres <command>
...
## What is Postgres?
Postgres (postgres) is a specialized tool/concept in the database domain.
It provides essential capabilities for professionals working with postgres.

## Key Concepts
- Core postgres principles and fundamentals
- How postgres fits into the broader database ecosystem  
- Essential terminology every practitioner should know

## Why Postgres Matters
Understanding postgres is critical for:
...
## Getting Started
1. Understand the basic postgres concepts
...
## Installation
1. Download or clone the postgres package
...
cmd_patterns() {
    cat << 'EOF'
# Postgres — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for postgres
...
cmd_migration() {
    cat << 'EOF'
# Postgres — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "postgres v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: postgres help"; exit 1 ;;
esac
